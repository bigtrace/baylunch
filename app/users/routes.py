...
from app.users.forms import (RegisterForm, LoginForm, UpdateAccounForm,
...
def register():
    ...
    if form_obj.validate_on_submit():
        ...
        user = Customer(username=form_obj.username.data,email=form_obj.email.data,password=hashed_pwd)
        db.session.add(user)
        db.session.commit()
...
@users.route("/sms/register",methods=['GET','POST'])
def register():
    if current_user.is_authenticated:
        return redirect(url_for('main.home'))
